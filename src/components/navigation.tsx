...
export function NavItem({ icon: Icon, title, href }: NavItemProps) {
  ...
  return (
    <Link
      to={href}
      className={cn(
        "group relative flex items-center rounded-xl transition-all duration-200 hover:scale-[1.02]",
        "bg-card border shadow-sm hover:shadow-md",
        isCollapsed ? "h-12 w-12 p-2 justify-center" : "h-12 w-full p-3 justify-start",
        isActive 

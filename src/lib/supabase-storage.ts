...
export const cookieStorage = {
  getItem: (key: string): string | null => {
    ...
    // Always use STORAGE_KEY for cross-subdomain compatibility
    const storageKey = key === 'bunting-auth-token' ? key : STORAGE_KEY;
    
    // Check for chunked storage
    const countStr = getCookie(`${storageKey}.count`);
    if (countStr) {
      ...
      console.log(`[BuntingAuth] Cookie read: reassembled ${count} chunks (${reassembled.length} chars)`);
      return reassembled;
    }
    
    // Fallback to single cookie
    const single = getCookie(storageKey);
    if (single) {
      console.log(`[BuntingAuth] Cookie read: single cookie (${single.length} chars)`);
    }
    return single;
  },
  ...
  setItem: (key: string, value: string): void => {
    ...
    const existingCount = getCookie(`${storageKey}.count`);
    if (existingCount) {
      const count = parseInt(existingCount, 10);
      for (let i = 0; i < count; i++) {
        deleteCookie(`${storageKey}.${i}`);
      }
      deleteCookie(`${storageKey}.count`);
    }
    ...
    if (encoded.length <= CHUNK_SIZE) {
      ...
      console.log(`[BuntingAuth] Cookie set: single cookie (${value.length} chars)`);
      return;
    }
    
    // Split into chunks (split the raw value, not encoded)
    const chunks: string[] = [];
    ...
    // Store chunks
    setCookie(`${storageKey}.count`, chunks.length.toString());
    chunks.forEach((chunk, i) => {
      setCookie(`${storageKey}.${i}`, chunk);
    });
    
    console.log(`[BuntingAuth] Cookie set: ${chunks.length} chunks (${value.length} chars)`);
  },
  ...
  removeItem: (key: string): void => {
    ...
    console.log(`[BuntingAuth] Cookie removed: ${storageKey}`);
  }
};

// Export storage key for reference

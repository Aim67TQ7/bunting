...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { action, badgeNumber, pin, otp } = body;
    ...
    if (action === 'lookup') {
      ...
      const { data: employee, error: empError } = await supabase
        .from('employees')
        .select('id, name_first, name_last, user_id, badge_pin_hash, reports_to, user_email')
    ...
    // =============================================================================
    // SIGNUP-REQUEST - Send OTP to supervisor for new badge user
    // =============================================================================
    if (action === 'signup-request') {
      ...
      // We'll use a simple approach - send via edge function email
...
If you did not expect this request, please contact IT.
    ...
    // =============================================================================
    // SIGNUP-VERIFY - Verify OTP and create account with PIN
    // =============================================================================
    ...
    if (action === 'login') {
      ...
      const { data: employee, error: empError } = await supabase
        .from('employees')
        .select('id, user_id, badge_pin_hash, badge_pin_attempts, badge_pin_locked_until')
        .eq('badge_number', badgeNumber)
        .maybeSingle();
